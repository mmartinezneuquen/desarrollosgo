    ...
    public function onLoad($param) {
        ...
        if (!$this->IsPostBack) {
            ...
            foreach ($items as $i => $item) {
                if ($this->Municipio)
                    $item->tcBotones->pnlAprobar->Visible = "false";
...
//        echo'<pre>';
//        print_r($documentación);
        $d = 0;
        foreach ($documentación as $doc) {
                            $textArchivo = '';

            if ($doc['Archivo'] != '') {
                $archivos = explode("||", $doc['Archivo']);
//                print_r($archivos);
                $i = 1;
                foreach ($archivos as $archivo) {
                    $textArchivo.=" <a href='output/documentos/$archivo' target='_blank'>Archivo $i</a> - ";
                    $i++;
                }
                if ($textArchivo != '')
                    $textArchivo = substr($textArchivo, 0, -3);
               
            }
             $documentación[$d]['Archivo'] = $textArchivo;
                $d++;
        }
//        print_r($documentación);
        $this->dgDatos->DataSource = $documentación;
        $this->dgDatos->dataBind();
//print_r($this->dgDatos->Items);
//        exit();
...
//                    exit();
        foreach ($this->dgDatos->Items as $it) {
            ...
            if ($uploadedFile)
                rename("$storeTmpPath/$uploadedFile", "$storePath/$uploadedFile");

            $rejected = explode(';', $it->tcBotones->hdnRejectedDocumentFiles->Value);
            if (sizeof($rejected) && $rejected[0]) {
                foreach ($rejected as $rejectedFile)
                    @unlink("$storeTmpPath/$rejectedFile");
            }
// //>> segundo Archivo
            $uploadedFile2 = $it->tcBotones->hdnDocumentFile2->Value;
            if ($uploadedFile2)
                rename("$storeTmpPath/$uploadedFile2", "$storePath/$uploadedFile2");

            $rejected2 = explode(';', $it->tcBotones->hdnRejectedDocumentFiles2->Value);
            if (sizeof($rejected2) && $rejected2[0]) {
                foreach ($rejected2 as $rejectedFile2)
                    @unlink("$storeTmpPath/$rejectedFile2");
            }
// //>> tercer Archivo            
            $uploadedFile3 = $it->tcBotones->hdnDocumentFile3->Value;
            if ($uploadedFile3)
                rename("$storeTmpPath/$uploadedFile3", "$storePath/$uploadedFile3");

            $rejected3 = explode(';', $it->tcBotones->hdnRejectedDocumentFiles3->Value);
            if (sizeof($rejected3) && $rejected3[0]) {
                foreach ($rejected3 as $rejectedFile3)
                    @unlink("$storeTmpPath/$rejectedFile3");
            }
// //>> cuarta Archivo            
            $uploadedFile4 = $it->tcBotones->hdnDocumentFile4->Value;
            if ($uploadedFile4)
                rename("$storeTmpPath/$uploadedFile4", "$storePath/$uploadedFile4");

            $rejected4 = explode(';', $it->tcBotones->hdnRejectedDocumentFiles4->Value);
            if (sizeof($rejected4) && $rejected4[0]) {
                foreach ($rejected4 as $rejectedFile4)
                    @unlink("$storeTmpPath/$rejectedFile4");
            }
            
// //>> quinta Archivo            
            $uploadedFile5 = $it->tcBotones->hdnDocumentFile5->Value;
            if ($uploadedFile4)
                rename("$storeTmpPath/$uploadedFile5", "$storePath/$uploadedFile5");

            $rejected5 = explode(';', $it->tcBotones->hdnRejectedDocumentFiles5->Value);
            if (sizeof($rejected5) && $rejected5[0]) {
                foreach ($rejected5 as $rejectedFile5)
                    @unlink("$storeTmpPath/$rejectedFile5");
            }
            ...
            if ($uploadedFile || $it->tcBotones->hdnAprobado->Value) {
                ...
                if ($uploadedFile)
                    $revision->IdEstado = 2;
                if ($it->tcBotones->hdnAprobado->Value == "aprobado")
                    $revision->IdEstado = 3;
                if ($it->tcBotones->hdnAprobado->Value == "desaprobado")
                    $revision->IdEstado = 4;
                $revision->Revision = 0;
...
//                $revision->Archivo = $uploadedFile ? $uploadedFile : $it->tcBotones->hdnCurrentDocumentFile->Value;
                $archivos = $uploadedFile ? $uploadedFile : $it->tcBotones->hdnCurrentDocumentFile->Value;


                if ($uploadedFile2)
                    $archivos.="||" . $uploadedFile2;
                if ($uploadedFile3)
                    $archivos.="||" . $uploadedFile3;
    if ($uploadedFile4)
                    $archivos.="||" . $uploadedFile4;
        if ($uploadedFile5)
                    $archivos.="||" . $uploadedFile5;
                $revision->Archivo = $archivos;
                try {
                    $revision->save();
                } catch (exception $e) {
